package com.magic.ioc.context;


import com.magic.ioc.annoation.WAutowired;
import com.magic.ioc.annoation.WService;
import com.magic.ioc.utils.ClassUtil;
...
public class WApplicationContext {
    ...
    public WApplicationContext(String path){
        this.path = path;
    }
    ...
    private void initAttribute(Object object)throws Exception{
        ...
        for(Field field : fields){
            //查找字段上有依赖的注解
            boolean falg = field.isAnnotationPresent(WAutowired.class);
            if (falg){
    ...
    public ConcurrentHashMap<String,Object>initBean(List<Class>classes) throws IllegalAccessException,InstantiationException{
        ...
        for(Class clazz :classes){
            ...
            WService annotation =(WService)clazz.getDeclaredAnnotation(WService.class);
            if (annotation!=null){
    ...
    private List<Class>findAnnoationService()throws Exception{
        ...
        if (classes==null || classes.size()==0){
            throw new Exception("not found service is added annoation for @WService");
        }
        List<Class> annoationClasses = new ArrayList<Class>();
        for (Class clazz:classes){
            //通过反射机制 查找增加了注解的类
            WService WService = (WService) clazz.getDeclaredAnnotation(WService.class);
            if (WService!=null){
